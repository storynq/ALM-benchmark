...
train_drop = train_data.dropna(subset = ['CDR1'])
valid_drop = valid_data.dropna(subset = ['CDR1'])
test_drop = test_data.dropna(subset = ['CDR1'])

train_filtered = train_drop[~(train_drop['Total_CDR'].str.rfind('1') > 166)]
valid_filtered = valid_drop[~(valid_drop['Total_CDR'].str.rfind('1') > 166)]
test_filtered = test_drop[~(test_drop['Total_CDR'].str.rfind('1') > 166)]

datasets = DatasetDict({
    "train": Dataset.from_pandas(train_filtered[['Sequence', 'Total_CDR', 'CDR1']]),
    "valid": Dataset.from_pandas(valid_filtered[['Sequence', 'Total_CDR', 'CDR1']]),
    "test": Dataset.from_pandas(test_filtered[['Sequence', 'Total_CDR', 'CDR1']])
})
...
dataset_tokenized = datasets.map(
    ...
    batched=True,
    batch_size=32,
    remove_columns=['Sequence', 'Total_CDR', 'CDR1']
)
