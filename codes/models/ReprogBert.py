...
class ModifiedBertLMPredictionHead(BertLMPredictionHead):
    def __init__(self, config):
        super().__init__(config)
        self.gamma = nn.Linear(config.vocab_size, 30, bias=False)
        self.gammabias = nn.Parameter(torch.zeros(30))
...
class ReprogClassficationHead(torch.nn.Module):
...
class BertForTokenClassficationProt(ModifiedBertPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
...
class BertForSequenceClassificationProt(ModifiedBertPreTrainedModel, BertForMaskedLM):
    def __init__(self, config, num_labels):
        ...
        # CHANGED: BertOnlyMLMHead -> ModifiedBertOnlyMLMHead
        self.cls = ReprogClassficationHead(num_labels)
